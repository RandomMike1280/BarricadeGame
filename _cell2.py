...
class MCTS:
    ...
    def _expand(self, node: SearchNode) -> SearchEvaluation:
        ...
        legal_tensor = torch.as_tensor(canonical_legal, dtype=torch.long, device=self.device)
    ...
    for ply in range(max_steps):
        ...
        samples.append(
            ReplaySample(
                ...
                mask=canonicalize_action_vector(state.action_mask(), side_to_move).cpu(),
                policy_target=canonicalize_action_vector(
                    result.policy_target, side_to_move
                ).cpu(),
